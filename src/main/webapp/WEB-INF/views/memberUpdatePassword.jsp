		...
		<div class="col-lg-9">
			...
			<div class="card mb-4">
				<div class="card-body">
					...
					<hr>
					<form action="/memberUpdatePassword" method="post">
					<h5>아이디 : ${member.memId }</h5>
					<hr>
					<h5>현 비밀번호 : <input type="password" name="memPassword"></h5>
					<hr>
					<h5>변경 비밀번호 : <input type="password" name="newMemPassword"></h5>
					<hr>
					<h5>변경 비밀번호  확인 : <input type="password" name="memPasswordOk"></h5>
					<hr>
					<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
					<input type="hidden" name="memNickname" value="${member.memNickname}" />
					<input type="hidden" name="memEmail" value="${member.memEmail}" />
					<input class="btn-xm btn-primary" type="submit" value="변경">
					</form>
				</div>
			</div>
		</div>
